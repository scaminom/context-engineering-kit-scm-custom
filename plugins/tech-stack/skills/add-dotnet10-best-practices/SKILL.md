---
name: tech-stack:add-dotnet10-best-practices
description: Emit .claude/rules/dotnet10-best-practices.md with ASP.NET Core 10 rules and register pointer in CLAUDE.md
argument-hint: Optional argument which practices to add or avoid
---

# Setup ASP.NET Core 10 Best Practices
...
Use the Write tool to create `.claude/rules/dotnet10-best-practices.md` with the following content, strictly as-is:
...
- If `CLAUDE.md` does NOT exist: create it with this minimal content:
  ...
  - `.claude/rules/dotnet10-best-practices.md` — ASP.NET Core 10 performance & reliability (auto-loads on `**/*.cs`)
  ```
